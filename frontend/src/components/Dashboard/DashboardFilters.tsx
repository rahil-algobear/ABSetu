...
export function DashboardFiltersBar({
  ...
  const filteredTypes = filters.activity_category_id
    ? allTypes?.filter((t) => t.category_id === filters.activity_category_id)
    : allTypes;

  // Clear type if category changes and type no longer matches
  useEffect(() => {
    if (
      ...
      const typeMatch = allTypes.find(
        (t) =>
          t.id === filters.activity_type_id &&
          t.category_id === filters.activity_category_id
      );
      if (!typeMatch) {
  ...
  return (
    <div className="mb-4">
      ...
      <div className="flex flex-wrap gap-2">
        {/* Activity Category */}
        <FilterSelect
          label={v("activity_category")}
          value={filters.activity_category_id || ""}
          ...
          onChange={(val) =>
            onChange({
              ...filters,
              activity_category_id: val || undefined,
              activity_type_id: val ? filters.activity_type_id : undefined,
            })
          }
        />

        {/* Activity Type (filtered by category) */}
        <FilterSelect
      ...
      {hasFilters && (
        <div className="flex flex-wrap gap-1.5 mt-3">
          {filters.activity_category_id && (
            <FilterChip
              label={
                categories?.find((c) => c.id === filters.activity_category_id)
                  ?.name ?? ""
              }
              onRemove={() =>
                onChange({
