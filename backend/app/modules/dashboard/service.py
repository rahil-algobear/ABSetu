...
from app.modules.dimension.model import ActivityTag
...
class DashboardService:
    ...
    def _apply_activity_filters(
        ...
        elif activity_category_id:
            if not need_type_join:
                query = query.join(ActivityType, Activity.activity_type_id == ActivityType.id)
            query = query.filter(ActivityType.category_id == activity_category_id)
        ...
        if dimension_value_ids:
            ...
            for dv_id in dimension_value_ids:
                query = query.filter(
                    Activity.id.in_(
                        self.db.query(ActivityTag.activity_id).filter(
                            ActivityTag.dimension_value_id == dv_id
                        )
                    )
                )
    ...
    def get_stats(
        ...
        activity_category_id: str | None = None,
        activity_type_id: str | None = None,
    ) -> DashboardStats:
        org_filter = {"organization_id": organization_id}
        has_filters = bool(dimension_value_ids or activity_category_id or activity_type_id)
        ...
        activity_count_q = self._apply_activity_filters(
            activity_count_q, dimension_value_ids, activity_category_id, activity_type_id
        )
        ...
        # --- Activities by category (with filters) ---
        cat_q = (
            self.db.query(
        ...
        cat_q = self._apply_activity_filters(
            cat_q, dimension_value_ids, activity_category_id, activity_type_id, need_type_join=True
        )
        activities_by_category_rows = (
            cat_q.group_by(ActivityCategory.name)
            .order_by(func.count(Activity.id).desc())
            .all()
        )
        activities_by_category = [CountByItem(label=name, count=cnt) for name, cnt in activities_by_category_rows]

        # --- Activities by type (with filters) — new breakdown ---
        type_q = (
            self.db.query(
                ActivityType.name,
                func.count(Activity.id),
        ...
        type_q = self._apply_activity_filters(
            type_q, dimension_value_ids, activity_category_id, activity_type_id, need_type_join=True
        )
        ...
        dim_q = (
            ...
            .join(DimensionValue, ActivityTag.dimension_value_id == DimensionValue.id)
            .join(Dimension, DimensionValue.dimension_id == Dimension.id)
            .filter(Activity.organization_id == organization_id)
        )
        dim_q = self._apply_activity_filters(
            dim_q, dimension_value_ids, activity_category_id, activity_type_id
        )
        ...
        time_q = self._apply_activity_filters(
            time_q, dimension_value_ids, activity_category_id, activity_type_id
        )
        ...
        recent_q = self._apply_activity_filters(
            recent_q, dimension_value_ids, activity_category_id, activity_type_id
        )
        recent_rows = (
            recent_q.order_by(Activity.date.desc(), Activity.created_at.desc())
            .limit(10)
            .all()
        )
